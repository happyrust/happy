...
export const ru: TranslationStructure = {
    ...
    connect: {
        ...
        authUrlHelpTitle: 'Не получается отсканировать QR-код?',
        authUrlHelpDescription: 'Скопируйте или поделитесь этой ссылкой аутентификации и откройте её на уже привязанном устройстве.',
        authUrlLabel: 'Ссылка для аутентификации',
        copyAuthUrl: 'Скопировать ссылку',
        shareAuthUrl: 'Поделиться ссылкой',
        authUrlCopied: 'Ссылка для аутентификации скопирована в буфер обмена',
        authUrlCopyFailed: 'Не удалось скопировать ссылку для аутентификации',
        shareAuthUrlFailed: 'Не удалось поделиться ссылкой для аутентификации',
    },
    ...
    settingsAutoMode: {
        ...
        autoModeGroup: 'Автоматический режим',
        autoModeFooter: 'При включении автоматически отправляет выбранное шаблонное сообщение, когда ИИ завершает ответ.',
        enableAutoMode: 'Включить автоматический режим',
        autoModeIsEnabled: 'Автоматический режим включен',
        autoModeIsDisabled: 'Автоматический режим отключен',
        
        cycleSettings: 'Настройки циклов',
        cycleSettingsFooterUnlimited: 'Неограниченные циклы - будет продолжаться автоматически до ручного отключения',
        cycleSettingsFooterLimited: ({ count }: { count: number }) => 
            `Максимум ${count} ${count === 1 ? 'цикл' : count < 5 ? 'цикла' : 'циклов'}. После достижения лимита автоматический режим остановится для этой сессии.`,
        maximumCycles: 'Максимум циклов (0 = неограничено)',
        enterNumber: 'Введите число (0 для неограниченных)',
        unlimitedCycles: 'Неограниченные циклы',
        maximumCyclesCount: ({ count }: { count: number }) => 
            `Максимум ${count} ${count === 1 ? 'цикл' : count < 5 ? 'цикла' : 'циклов'}`,
        currentCycleCounts: 'Текущие счётчики циклов',
        sessionLabel: ({ id }: { id: string }) => `Сессия: ${id.substring(0, 8)}...`,
        cycleProgress: ({ current, max }: { current: number; max: string }) => `${current} / ${max}`,
        resetCycleCount: 'Сбросить счётчик циклов',
        resetCycleCountMessage: 'Сбросить счётчик циклов для этой сессии?',
        resetAllCycleCounts: 'Сбросить все счётчики циклов',
        resetAllCycleCountsMessage: 'Сбросить счётчики циклов для всех сессий?',
        resetAll: 'Сбросить все',
        
        selectedTemplate: 'Выбранный шаблон',
        selectedTemplateFooter: ({ name }: { name: string }) => `Текущий шаблон: "${name}"`,
        noTemplateSelected: 'Шаблон не выбран. Добавьте шаблон ниже, чтобы начать.',
        noTemplateTitle: 'Шаблон не выбран',
        selectTemplateSubtitle: 'Выберите шаблон из списка ниже',
        
        templates: 'Шаблоны',
        templatesFooter: 'Шаблоны автоматически отправляются, когда ИИ завершает ответ (если включён автоматический режим).',
        noTemplates: 'Нет шаблонов',
        addFirstTemplate: 'Добавьте свой первый шаблон ниже',
        
        addEditTemplate: ({ isEditing }: { isEditing: boolean }): string => isEditing ? 'Редактировать шаблон' : 'Добавить шаблон',
        templateName: 'Название шаблона',
        enterTemplateName: 'Введите название шаблона',
        templateContent: 'Содержимое шаблона',
        enterTemplateContent: 'Введите содержимое шаблона (будет отправлено автоматически)',
        templateNameRequired: 'Название и содержимое шаблона обязательны',
        
        deleteTemplate: 'Удалить шаблон',
        deleteTemplateMessage: 'Вы уверены, что хотите удалить этот шаблон?',
        delete: 'Удалить',
        cancel: 'Отмена',
        save: 'Сохранить',
    ...
    sessionInfo: {
        ...
        autoModeDescriptionDefault: ({ globalState }: { globalState: string }) => `Автоматический режим по умолчанию отключен для этой сессии. Глобальная настройка: ${globalState}. Включите вручную для переопределения.`,
        autoModeDisabledDefault: 'Автоматический режим отключен (по умолчанию, можно включить вручную)',
        ...
        autoModeSessionMaxLabel: 'Лимит циклов для сессии',
        autoModeSessionMaxSet: ({ count }: { count: number }) => `Лимит ${count} ${count === 1 ? 'цикла' : 'циклов'} для этой сессии`,
        autoModeSessionMaxUnlimited: 'Для этой сессии нет ограничения по циклам',
        autoModeSessionMaxGlobal: ({ count }: { count: number }) => `Используется глобальный лимит ${count} ${count === 1 ? 'цикла' : 'циклов'}`,
        autoModeSessionMaxGlobalUnlimited: 'Используется глобальная настройка без ограничений',
        autoModeSessionMaxPromptTitle: 'Задать лимит циклов для сессии',
        autoModeSessionMaxPromptMessage: 'Введите максимальное число циклов автоматического режима для этой сессии (0 — без ограничений). Оставьте пустым, чтобы использовать глобальную настройку.',
        autoModeSessionMaxPromptPlaceholder: 'например, 5',
        autoModeSessionMaxInvalid: 'Введите целое неотрицательное число.',
        customAutoModeMessage: 'Пользовательское сообщение автоотправки',
        customAutoModeMessageEmpty: 'Нет пользовательского сообщения. Нажмите, чтобы установить.',
        customAutoModeMessageTitle: 'Пользовательское сообщение автоотправки',
        customAutoModeMessagePrompt: 'Введите пользовательское сообщение для автоотправки после ответа ИИ. Оставьте пустым для использования шаблонов.',
        customAutoModeMessagePlaceholder: 'например, Продолжить следующую задачу',
        clearCustomMessage: 'Очистить пользовательское сообщение',
        clearCustomMessageConfirm: 'Это удалит пользовательское сообщение и будет использоваться выбранный шаблон.',
        clearCustomMessageSubtitle: 'Использовать шаблон',
        autoModeTemplateLabel: 'Шаблон автоматического режима',
        ...
        restartSessionSubtitle: 'Удаленно перезапустить эту сессию на вашей машине',
        restartSessionSuccess: 'Сессия перезапускается...',
        restartSessionFailed: 'Не удалось перезапустить сессию',
        restartSessionMachineOffline: 'Машина offline. Сначала запустите daemon на вашем компьютере.',
        restartingSession: 'Перезапуск сессии...',
    ...
    agentInput: {
        templatePicker: 'Шаблоны автоответа',
        selectTemplate: 'Выберите шаблон',
        noTemplatesAvailable: 'Нет доступных шаблонов',
        currentMessage: 'Текущее сообщение',
        setAsAutoSend: 'Установить как автоотправку',
        saveAsTemplate: 'Сохранить как шаблон',
        enterTemplateName: 'Введите название шаблона',
        templateNamePlaceholder: 'например, Продолжить работу',
        templates: 'Шаблоны',
        repeatSend: 'Повторить отправку',
        repeatSendOnce: 'Отправить 1 раз',
        repeatSendTimes: ({ times }: { times: number }) => `Отправить ${times} ${times === 1 ? 'раз' : times < 5 ? 'раза' : 'раз'}`,
        repeatSendMenuTitle: 'Выберите количество повторов',
        customRepeatOption: 'Свое количество...',
        customRepeatTitle: 'Свое количество повторов',
        customRepeatMessage: 'Введите, сколько раз отправить это сообщение',
        customRepeatPlaceholder: 'например, 20',
        customRepeatInvalid: 'Введите положительное целое число',
        permissionMode: {
