...
export const pl: TranslationStructure = {
    ...
    connect: {
        ...
        authUrlHelpTitle: 'Nie możesz zeskanować kodu QR?',
        authUrlHelpDescription: 'Skopiuj lub udostępnij ten link uwierzytelniający i otwórz go na powiązanym urządzeniu.',
        authUrlLabel: 'Link uwierzytelniający',
        copyAuthUrl: 'Skopiuj link',
        shareAuthUrl: 'Udostępnij link',
        authUrlCopied: 'Link uwierzytelniający skopiowany do schowka',
        authUrlCopyFailed: 'Nie udało się skopiować linku uwierzytelniającego',
        shareAuthUrlFailed: 'Nie udało się udostępnić linku uwierzytelniającego',
    },
    ...
    settingsAutoMode: {
        ...
        autoModeGroup: 'Tryb Automatyczny',
        autoModeFooter: 'Po włączeniu automatycznie wysyła wybraną wiadomość szablonową, gdy AI zakończy odpowiadanie.',
        enableAutoMode: 'Włącz Tryb Automatyczny',
        autoModeIsEnabled: 'Tryb automatyczny jest włączony',
        autoModeIsDisabled: 'Tryb automatyczny jest wyłączony',
        
        cycleSettings: 'Ustawienia Cykli',
        cycleSettingsFooterUnlimited: 'Nieograniczone cykle - będzie kontynuować automatycznie, dopóki nie zostanie ręcznie wyłączony',
        cycleSettingsFooterLimited: ({ count }: { count: number }) => 
            `Maksymalnie ${count} ${count === 1 ? 'cykl' : count < 5 ? 'cykle' : 'cykli'}. Po osiągnięciu limitu tryb automatyczny zatrzyma się dla tej sesji.`,
        maximumCycles: 'Maksymalna Liczba Cykli (0 = nieograniczona)',
        enterNumber: 'Wpisz liczbę (0 dla nieograniczonej)',
        unlimitedCycles: 'Nieograniczone cykle',
        maximumCyclesCount: ({ count }: { count: number }) => 
            `Maksymalnie ${count} ${count === 1 ? 'cykl' : count < 5 ? 'cykle' : 'cykli'}`,
        currentCycleCounts: 'Bieżące Liczniki Cykli',
        sessionLabel: ({ id }: { id: string }) => `Sesja: ${id.substring(0, 8)}...`,
        cycleProgress: ({ current, max }: { current: number; max: string }) => `${current} / ${max}`,
        resetCycleCount: 'Zresetuj Licznik Cykli',
        resetCycleCountMessage: 'Zresetować licznik cykli dla tej sesji?',
        resetAllCycleCounts: 'Zresetuj Wszystkie Liczniki Cykli',
        resetAllCycleCountsMessage: 'Zresetować liczniki cykli dla wszystkich sesji?',
        resetAll: 'Zresetuj Wszystko',
        
        selectedTemplate: 'Wybrany Szablon',
        selectedTemplateFooter: ({ name }: { name: string }) => `Aktualny szablon: "${name}"`,
        noTemplateSelected: 'Nie wybrano szablonu. Dodaj szablon poniżej, aby rozpocząć.',
        noTemplateTitle: 'Nie wybrano szablonu',
        selectTemplateSubtitle: 'Wybierz szablon z listy poniżej',
        
        templates: 'Szablony',
        templatesFooter: 'Szablony są automatycznie wysyłane, gdy AI zakończy odpowiadanie (jeśli tryb automatyczny jest włączony).',
        noTemplates: 'Brak szablonów',
        addFirstTemplate: 'Dodaj swój pierwszy szablon poniżej',
        
        addEditTemplate: ({ isEditing }: { isEditing: boolean }): string => isEditing ? 'Edytuj Szablon' : 'Dodaj Szablon',
        templateName: 'Nazwa Szablonu',
        enterTemplateName: 'Wprowadź nazwę szablonu',
        templateContent: 'Zawartość Szablonu',
        enterTemplateContent: 'Wprowadź zawartość szablonu (będzie wysłana automatycznie)',
        templateNameRequired: 'Nazwa i zawartość szablonu są wymagane',
        
        deleteTemplate: 'Usuń Szablon',
        deleteTemplateMessage: 'Czy na pewno chcesz usunąć ten szablon?',
        delete: 'Usuń',
        cancel: 'Anuluj',
        save: 'Zapisz',
    ...
    sessionInfo: {
        ...
        autoModeDescriptionDefault: ({ globalState }: { globalState: string }) => `Tryb automatyczny jest domyślnie wyłączony dla tej sesji. Ustawienie globalne: ${globalState}. Włącz ręcznie, aby nadpisać.`,
        autoModeDisabledDefault: 'Tryb automatyczny jest wyłączony (domyślnie, można włączyć ręcznie)',
        ...
        autoModeSessionMaxLabel: 'Limit cykli dla sesji',
        autoModeSessionMaxSet: ({ count }: { count: number }) => `Limit ${count} ${count === 1 ? 'cyklu' : 'cykli'} dla tej sesji`,
        autoModeSessionMaxUnlimited: 'Ta sesja ma nieograniczoną liczbę cykli',
        autoModeSessionMaxGlobal: ({ count }: { count: number }) => `Używany globalny limit ${count} ${count === 1 ? 'cyklu' : 'cykli'}`,
        autoModeSessionMaxGlobalUnlimited: 'Używana globalna konfiguracja bez limitu',
        autoModeSessionMaxPromptTitle: 'Ustaw limit cykli dla sesji',
        autoModeSessionMaxPromptMessage: 'Podaj maksymalną liczbę cykli trybu automatycznego dla tej sesji (0 oznacza bez limitu). Pozostaw puste, aby użyć ustawienia globalnego.',
        autoModeSessionMaxPromptPlaceholder: 'np. 5',
        autoModeSessionMaxInvalid: 'Wprowadź nieujemną liczbę całkowitą.',
        customAutoModeMessage: 'Niestandardowa wiadomość automatyczna',
        customAutoModeMessageEmpty: 'Brak niestandardowej wiadomości. Dotknij, aby ustawić.',
        customAutoModeMessageTitle: 'Niestandardowa wiadomość automatyczna',
        customAutoModeMessagePrompt: 'Wprowadź niestandardową wiadomość do automatycznego wysłania po zakończeniu odpowiedzi AI. Pozostaw puste, aby użyć szablonów.',
        customAutoModeMessagePlaceholder: 'np. Kontynuuj następne zadanie',
        clearCustomMessage: 'Wyczyść niestandardową wiadomość',
        clearCustomMessageConfirm: 'Spowoduje to usunięcie niestandardowej wiadomości i użycie wybranego szablonu.',
        clearCustomMessageSubtitle: 'Użyj szablonu zamiast tego',
        autoModeTemplateLabel: 'Szablon trybu automatycznego',
        ...
        restartSessionSubtitle: 'Zdalnie uruchom ponownie tę sesję na swojej maszynie',
        restartSessionSuccess: 'Sesja jest ponownie uruchamiana...',
        restartSessionFailed: 'Nie udało się ponownie uruchomić sesji',
        restartSessionMachineOffline: 'Maszyna jest offline. Proszę najpierw uruchomić daemon na swoim komputerze.',
        restartingSession: 'Ponowne uruchamianie sesji...',
    ...
    agentInput: {
        templatePicker: 'Szablony automatycznej odpowiedzi',
        selectTemplate: 'Wybierz szablon',
        noTemplatesAvailable: 'Brak dostępnych szablonów',
        currentMessage: 'Aktualna wiadomość',
        setAsAutoSend: 'Ustaw jako wiadomość automatyczną',
        saveAsTemplate: 'Zapisz jako szablon',
        enterTemplateName: 'Wprowadź nazwę szablonu',
        templateNamePlaceholder: 'np. Kontynuuj pracę',
        templates: 'Szablony',
        repeatSend: 'Powtórz wysyłanie',
        repeatSendOnce: 'Wyślij 1 raz',
        repeatSendTimes: ({ times }: { times: number }) => `Wyślij ${times} ${times === 1 ? 'raz' : times < 5 ? 'razy' : 'razy'}`,
        repeatSendMenuTitle: 'Wybierz liczbę powtórzeń',
        customRepeatOption: 'Własna liczba...',
        customRepeatTitle: 'Własna liczba powtórzeń',
        customRepeatMessage: 'Wprowadź liczbę wysłań tej wiadomości',
        customRepeatPlaceholder: 'np. 20',
        customRepeatInvalid: 'Wprowadź dodatnią liczbę całkowitą',
        permissionMode: {
