...
export const zhHans: TranslationStructure = {
    ...
    settingsAutoMode: {
        title: '自动模式',
        autoModeGroup: '自动模式',
        autoModeFooter: '启用后，当 AI 完成响应时会自动发送选定的模板消息。',
        enableAutoMode: '启用自动模式',
        autoModeIsEnabled: '自动模式已启用',
        autoModeIsDisabled: '自动模式已禁用',
        
        cycleSettings: '循环设置',
        cycleSettingsFooterUnlimited: '无限循环 - 将自动继续，直到手动禁用',
        cycleSettingsFooterLimited: ({ count }: { count: number }) => 
            `最多 ${count} 个循环。达到限制后，此会话的自动模式将停止。`,
        maximumCycles: '最大循环次数（0 = 无限）',
        enterNumber: '输入数字（0 表示无限）',
        unlimitedCycles: '无限循环',
        maximumCyclesCount: ({ count }: { count: number }) => 
            `最多 ${count} 个循环`,
        currentCycleCounts: '当前循环计数',
        sessionLabel: ({ id }: { id: string }) => `会话：${id.substring(0, 8)}...`,
        cycleProgress: ({ current, max }: { current: number; max: string }) => `${current} / ${max}`,
        resetCycleCount: '重置循环计数',
        resetCycleCountMessage: '重置此会话的循环计数？',
        resetAllCycleCounts: '重置所有循环计数',
        resetAllCycleCountsMessage: '重置所有会话的循环计数？',
        resetAll: '全部重置',
        
        selectedTemplate: '已选模板',
        selectedTemplateFooter: ({ name }: { name: string }) => `当前模板："${name}"`,
        noTemplateSelected: '未选择模板。请在下方添加模板以开始。',
        noTemplateTitle: '未选择模板',
        selectTemplateSubtitle: '从下方列表中选择一个模板',
        
        templates: '模板',
        templatesFooter: '当 AI 完成响应时自动发送模板（如果启用了自动模式）。',
        noTemplates: '无模板',
        addFirstTemplate: '在下方添加您的第一个模板',
        
        addEditTemplate: ({ isEditing }: { isEditing: boolean }): string => isEditing ? '编辑模板' : '添加模板',
        templateName: '模板名称',
        enterTemplateName: '输入模板名称',
        templateContent: '模板内容',
        enterTemplateContent: '输入模板内容（将自动发送）',
        templateNameRequired: '模板名称和内容为必填项',
    ...
    sessionInfo: {
        ...
        autoModeDescriptionDefault: ({ globalState }: { globalState: string }) => `此会话默认禁用自动模式。全局设置：${globalState}。可手动启用以覆盖。`,
        autoModeDisabledDefault: '自动模式已禁用（默认，可手动启用）',
        ...
        autoModeTemplateLabel: '自动模式模板',
        autoModeTemplateEmpty: '暂无模板。请前往 设置 > 自动模式 创建模板。',
        autoModeTemplatePickerTitle: '选择自动模式模板',
        autoModeTemplatePickerSubtitle: '选择在自动模式运行时需要自动发送的模板内容。',
        autoModeSessionMaxLabel: '会话循环上限',
        autoModeSessionMaxSet: ({ count }: { count: number }) => `此会话最多循环 ${count} 次`,
        autoModeSessionMaxUnlimited: '此会话无限循环',
        autoModeSessionMaxGlobal: ({ count }: { count: number }) => `使用全局上限：${count} 次`,
        autoModeSessionMaxGlobalUnlimited: '使用全局无限循环设置',
        autoModeSessionMaxPromptTitle: '设置会话循环上限',
        autoModeSessionMaxPromptMessage: '输入此会话自动模式的最大循环次数（0 表示无限）。留空以使用全局设置。',
        autoModeSessionMaxPromptPlaceholder: '例如 5',
        autoModeSessionMaxInvalid: '请输入不小于 0 的整数。',
        customAutoModeMessage: '自定义自动发送消息',
        customAutoModeMessageEmpty: '无自定义消息。点击设置自动发送的自定义消息。',
        customAutoModeMessageTitle: '自定义自动发送消息',
        customAutoModeMessagePrompt: '输入在 AI 回复结束时自动发送的自定义消息。留空则使用模板。',
        customAutoModeMessagePlaceholder: '例如：继续下一个任务',
    ...
    agentInput: {
        templatePicker: '自动回复模板',
        selectTemplate: '选择模板',
        noTemplatesAvailable: '暂无可用模板',
        currentMessage: '当前消息',
        setAsAutoSend: '设为自动发送消息',
